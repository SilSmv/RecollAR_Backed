...
public interface ObjectRepository extends JpaRepository<ObjectModel,Integer> {
    ...
    @Query(value = "select o.id_object idObject,o.name name, o.image image  from object o INNER JOIN collection c on c.id_collection=o.id_collection where o.id_collection = ?1 and o.status=1 and c.status=1 and c.id_collector=?2 order by o.id_object DESC", nativeQuery = true)
    ...
    @Query(value = "select o.* from object o INNER JOIN collection c on c.id_collection=o.id_collection where o.id_object= ?1 and o.status=1 and c.status=1 and c.id_collector=?2", nativeQuery = true)
